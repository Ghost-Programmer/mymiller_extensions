...
/**
 * Created by User: hansolo Date: 01.01.13 Time: 15:50
 */
public class Helper {
    ...
    public static String getDMSCompass(final double LATITUDE, final double LONGITUDE) {
        ...
        DIRECTION.append(Math.abs(LATITUDE)).append(" ").append(Helper.getLatitudeDirection(LATITUDE));
        DIRECTION.append(", ");
        DIRECTION.append(Math.abs(LONGITUDE)).append(" ").append(Helper.getLongitudeDirection(LONGITUDE));
    ...
    public static Point2D latLongToPixel(final Dimension2D MAP_DIMENSION, final Point2D UPPER_LEFT,
                                         final Point2D LOWER_RIGHT, final Point2D LOCATION) {
        ...
        final double MAP_OFFSET_Y = ((WORLD_MAP_WIDTH / 2)
                * Math.log10((1 + Math.sin(Math.toRadians(LOWER_RIGHT.getX())))
        ...
        final double Y = MAP_HEIGHT - (((WORLD_MAP_WIDTH / 2)
                * Math.log10((1 + Math.sin(Math.toRadians(LATITUDE))) / (1 - Math.sin(Math.toRadians(LATITUDE)))))
                - MAP_OFFSET_Y);
